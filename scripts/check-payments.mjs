/**
 * Exercises the payment webhooks against a real running server.
 *
...
const PADDLE_SECRET = "pdl_ntfset_cutframe_local_test";
...
try {
  ...
  console.log("\nPaddle — rejecting anything that isn't Paddle:");
  ...
  function paddleSign(body, secret = PADDLE_SECRET, ts = Math.floor(Date.now() / 1000)) {
    const h1 = createHmac("sha256", secret).update(`${ts}:${body}`).digest("hex");
    return `ts=${ts};h1=${h1}`;
  }

  async function paddlePost(body, signature) {
  ...
  check("no signature header", await paddlePost(paddleBody, undefined), 400);
  check("garbage signature", await paddlePost(paddleBody, "nonsense"), 400);
  check("signed with the wrong secret", await paddlePost(paddleBody, paddleSign(paddleBody, WRONG_SECRET)), 400);

  const paddleSigned = paddleSign(paddleBody);
  const paddleTampered = paddleBody.replace('"400"', '"999999"');
  check("body tampered with after signing", await paddlePost(paddleTampered, paddleSigned), 400);

  const staleTs = Math.floor(Date.now() / 1000) - 3600;
  check("signature older than the tolerance", await paddlePost(paddleBody, paddleSign(paddleBody, PADDLE_SECRET, staleTs)), 400);

  console.log("\nPaddle — answering correctly:");

  const otherEvent = JSON.stringify({ event_type: "transaction.created", data: { id: "txn_x" } });
  check("an event type we ignore", await paddlePost(otherEvent, paddleSign(otherEvent)), 200);

  const noCustom = JSON.stringify({
    event_type: "transaction.completed",
    data: { id: "txn_nocustom", status: "completed", custom_data: {} },
  });
  check("a completed sale we can't attribute", await paddlePost(noCustom, paddleSign(noCustom)), 200);

  // Properly signed, attributable, but the account doesn't exist. Must fail
  // loudly — a 200 here would tell Paddle the money was delivered.
  check("a real sale we cannot credit", await paddlePost(paddleBody, paddleSigned), 500);

  console.log();
  if (failures === 0) {
    console.log("All payment webhook checks passed.");
  } else {
    console.log(`${failures} payment webhook check(s) failed — do not deploy.`);
  }
} finally {
  server.kill("SIGTERM");
}
